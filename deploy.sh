...
#Todo: doesnot replace sshtoken

sshKeyData=$(cat ~/.ssh/azure_rsa.pub)
#parameterJson=$( echo "$parameterJson"  | jq "{sshKeyData: {value: "\"$sshKeyData"\"}} + ." )
...
--parameters clientSecret="@clientsecret.json" \
--parameters _artifactsLocation=$location --verbose \
--parameters 
echo $template
echo $templateUri 
